 ...
 *   CYYU@nuvoton.com                                                      *
 * 																		   *
 *   Copyright (C) 2022 KunYi Chen                                         *
 *   kunyi.chen+openocd@gmail.com                                          *
 *                                                                         *
...
#define NUMICRO_SPROM_BASE2     0x00240000
#define NUMICRO_SPROM_BASE3     0x00280000
#define NUMICRO_CONFIG_BASE     0x00300000
#define NUMICRO_DATA_DFMC_BASE  0x00400000
#define NUMICRO_SPECIAL_FLASH_OFFSET 0x0F000000
...
#define NUMICRO_SPROM_MINI57_MASK 0x00000002
#define NUMICRO_FLASH_OFFSET_MASK 0x00000004
...
/* M23 MFC ISP */
#define NUMICRO_M23_FMC_ISPSTS			0x4000C040
#define NUMICRO_M23_FMC_ISPSTS_VECMAP	0x00FFFE00
#define NUMICRO_M460_FMC_ISPSTS_VECMAP	0x0FFFFE00
...
*/
		...
		m_addressMinusOffset = 0; // TODO: KunYi, avoid
/* TODO: KunYi
	...
	if (strcmp(m_target_name, "M480") == 0 && bSPIMFlashWrite) {
		...
		if (strcmp(m_target_name, "M471") == 0 && bDFMCFlashWrite) {	// M471 data flash DFMC erase
			...
			for (i = first; i <= last; i++) {
				...
				LOG_DEBUG("address: 0x%x, bank->base: 0x" TARGET_ADDR_FMT 
						  ", bank->sectors[i].offset: 0x%x,  %d",
						  address, bank->base, bank->sectors[i].offset, i);
				address = (bank->base + bank->sectors[i].offset);
				buf_set_u32(reg_params[0].value, 0, 32, address);
		...
		uint32_t offset, uint32_t count)
{
	...
	if ((retval == ERROR_TARGET_RESOURCE_NOT_AVAILABLE) &&
		...
		for (uint32_t i = 0; i < count; i += 4) {
			...
			retval = target_write_u32(target, NUMICRO_FLASH_ISPADR - m_addressMinusOffset,
										(bank->base + offset + i) & NUMICRO_TZ_MASK);
			if (retval != ERROR_OK)
				return retval;
	...
	if (m_M23SecureDebugState == NUMICRO_M23_SECURE_DEBUG_S) {
		...
		LOG_DEBUG("base: 0x" TARGET_ADDR_FMT ",offset: 0x%" PRIx32 "", bank->base, offset);
		if (bank->base + offset < NUMICRO_LDROM_BASE) {
			// APROM
...
static int numicro_get_cpu_type(struct target *target, const struct numicro_cpu_type **cpu)
{
	...
	*cpu = &numicro_parts[ARRAY_SIZE(numicro_parts) - 1];
	LOG_INFO("Device Name: %s", (*cpu)->partname);
